...
for (number.of.eigen in c(25, 30))
{
  for (kernel in c('linear', 'sigmoid', 'radial'))
  {
    for (mirror in c(FALSE, TRUE))
    {
      for (number.of.classfiers in c(100,150,200))
      {
        #for (number.of.classes.per.features.set in c(10))
        {
          samples.count <- 100
          ...
          name <- paste(main.path, 'results\\','\\svm.kernel=',kernel,
